...
const MessagePage = () => {
	...
	const guideSection = (text1, text2, buttonText) => <>
		<p className="font-bold text-[32px]">{text1}</p>
		<p className="text-[15px] text-slate-500">{text2}</p>
		<div className="mt-[25px] purple-button rounded-full w-max px-9 py-4 text-white font-bold cursor-pointer hover:bg-violet-800 transition-all">{buttonText}</div>
	</>
		...
		<div className="flex">
			<div className="border-l border-r border-color w-[395px]">
				...
				<div className="w-[80%] mx-auto mt-[30px]">
					{guideSection('Welcome to your inbox!', 'Drop a line, share posts and more with private conversations between you and others on X.', 'Write a message')}
				</div>
			</div>
			<div className="border-r border-color w-[600px]">
				<div className="w-[60%] mx-auto top-[50%] relative translate-y-[-50%]">
					{guideSection('Select a message', 'Choose from your existing conversations, start a new one, or just keep swimming.', 'New message')}
				</div>
			</div>
		</div>
	)
